/*******************************************************
 *  FULL AUTOMATIC CHATGPT POETRY SENDER – VERSION A   *
 *  Each item = one بيت (two hemistichs together)      *
 *  Paste directly into ChatGPT browser console        *
 *******************************************************/
...
const rhetoricBatches = [
  "الجناس", "تشبيه", "تكرار",
  "رد الأعجاز على الصدور", "طباق", "كناية",
  "مجاز", "مبالغة", "التفات",
  "المراجعة", "مقابلة", "تضمين (جريان)",
  "الرجوع", "تجريد", "التقسيم"
];
...
function run() {
    if (verseIndex >= poem.length) {
        console.log("✔️ Finished sending all verses!");
        return;
    }

    const verse = poem[verseIndex];
    const batch = rhetoricBatches.slice(batchIndex, batchIndex + 3);

    if (batch.length === 0) {
        // الانتهاء من جميع دفعات البيت الحالي، الانتقال للبيت التالي
        verseIndex++;
        batchIndex = 0;
        run();
        return;
    ...
    // الرسالة بصيغة {رقم البيت | البيت | عناصر الدفعة}
    const message = `{${verseIndex + 1} | ${verse} | ${batch.join(", ")}}`;
    console.log("➡️ Sending batch:", message);
